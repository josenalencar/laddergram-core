...
| `export.js` | `layoutLadder` (one ladder → editor points/connections, carrying each dot's `timeHandle` where its position IS a parameter), `toLewisLadderDiagram`, `toLineStyle`, the generator's own JSON |
